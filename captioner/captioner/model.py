...
class Model(nn.Module):
    ...
    def forward(self, feat, cap=None, nlp=None, max_len=20):
        ...
        if isinstance(self.rnn, nn.LSTM): h0 = (h0, torch.zeros_like(h0).to(mag.device))
    ...
    def _generate(self, h, nlp, max_len):
        y = torch.zeros(1, max_len).to(mag.device)
        x = torch.tensor(nlp('`')[0].vector).view(1, 1, -1).to(mag.device)
    ...
    def _sample(self, x, h, nlp):
        ...
        if idx == self.fc.out_features - 1: 
            next_vector = torch.zeros(1, 1, embedding_dim).to(mag.device)
        else:
            hash_val = nlp.vocab.vectors.find(row=idx.item())[0]
            next_vector = torch.tensor(nlp.vocab.vectors[hash_val]).to(mag.device).view(1, 1, -1)
